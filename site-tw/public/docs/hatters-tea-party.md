...
**4. The evidence itself is checked — and the check can be re-run from the bytes.** Pinning a source proves *where* a citation came from; it doesn't make the *content* safe. The **Oracle & Privacy Rails** close that gap: external research is untrusted input, so before it enters the chain a deterministic guardrail envelope quarantines unsafe-URL and obviously-poisoned results, a prompt-injection rail (Llama Prompt Guard 2) refuses crafted instructions hidden in a snippet, and a PII rail (Presidio) refuses contact/identity/secret data. These are the Hatter-side counterpart to the Red Queen: she governs what an agent may *do* at the tool boundary; the rails govern what evidence may *enter* the planning chain. And the trust model is **replay, not sign** — the closeout re-runs the exact pinned model over the committed bytes and compares to what was recorded at merge. A mismatch fails the rollup; it can never quietly pass. The same evidence a reviewer saw is what an auditor re-derives later, model pinned to a specific commit. *(A fourth rail — groundedness, proving each conclusion is entailed by its cited source — is the next act.)*
