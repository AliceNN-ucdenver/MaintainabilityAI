...
**Knight's Seal — a cryptographic signature on top of the chain.** Coming next: each agent dispatch generates a one-shot Ed25519 signing key, signs the chain-root hash + the artifact's content hash with it, posts the public key + signature in the audit log, then destroys the private key. The next step before merge re-verifies the signature. Looking Glass shows a *"Phase seal-checked ✓"* badge on each phase card — at a glance, a reviewer sees the chain is intact **and** the signature traces back to that specific agent session. If anyone tampers with the artifact or the chain after the run finishes, the badge flips to red and the merge refuses. (Future evolution: cosign / sigstore so a third party can verify a year-old artifact without trusting the public key embedded in its own audit log.)
