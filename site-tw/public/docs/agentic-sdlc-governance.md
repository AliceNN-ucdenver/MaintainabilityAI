<div class="docs-hero docs-hero-split docs-hero-indigo">
  ...
  <div class="docs-hero-inner">
    ...
    <div class="docs-eyebrow">Vision · The agentic SDLC governance framework <span class="docs-hero-meta">~14 min read</span></div>
    <h1 class="docs-hero-title">An agentic governed SDLC</h1>
    <p class="docs-hero-copy">
      AI agents write 70% of the code well. The other 30% — architecture, security posture, threat awareness, governance compliance — is where systems live or die. <strong>MaintainabilityAI gives both humans and agents the architectural map, the planning rails, and the enforcement gates to ship governed code on purpose.</strong>
    </p>
    <p class="docs-hero-copy">
...
AI agents generate code at unprecedented speed. Claude writes microservices in minutes. Copilot scaffolds entire applications in seconds. GitHub Actions chains them together autonomously. But the agents have **no idea what they're building** — they don't know your architecture, don't understand your threat model, can't see your governance scores, don't know that Service A must never directly access the database, or that your payment system requires PCI-DSS compliance.
...
The architecture decisions, security posture, threat awareness, and governance compliance — that 30% is where systems fail, breaches happen, and technical debt compounds into organizational debt. **MaintainabilityAI closes that gap by making governance visible to the people AND the agents doing the work.**

---

## The framework — end to end

Three things make the framework coherent: a **substrate** (Looking Glass) that everyone reads from, a **planning modality** (Hatter's Tea Party) that turns intent into governed artifacts, and an **enforcement modality** (Red Queen's Court) that gates what actually ships. Every artifact carries provenance; every gate is auditable.
...
<svg viewBox="0 0 800 460" xmlns="http://www.w3.org/2000/svg" class="docs-svg">
  ...
  <!-- Pillar 2: Red Queen's Court (enforcement) -->
  ...
  <!-- Looking Glass → both pillars (substrate feeds both) -->
  ...
  <!-- Hand-off arrow between the two -->
...
Looking Glass is the **substrate** — the CALM architecture model, STRIDE threat library, ADRs, NIST controls, and four-pillar governance scores that everything else reads from. The Hatter and the Red Queen are the **two modalities** of governance applied on top of that substrate.

> 🎩 **Hatter's Tea Party — plan, govern intent, ship audit-grade artifacts.**
> [Read the deep dive →](/docs/hatters-tea-party)

> ♛ **Red Queen's Court — enforce architecture in code, at the moment of action.**
> [Read the deep dive →](/docs/red-queens-court)
...
## Two modalities — pick your entry point

<div class="docs-grid docs-grid-wide">
  <div class="docs-card docs-card-indigo">
    <div class="docs-heading">🎩 The Hatter's Tea Party</div>
    <div class="docs-copy"><strong>Governs intent upstream of code.</strong> An OKR triggers market research grounded in four oracles (web · academic · patents · community) plus JTBD analysis. The research grounds a PRD refined by mesh-anchored "ask-experts." The PRD grounds a cross-cutting code design that's reviewed against the actual indexed repos — the heaviest gate in the pipeline. One Audit Report Export bundles the whole thread for a CIO to read in one sitting.</div>
    <div class="docs-copy"><a href="/docs/hatters-tea-party" class="docs-button-secondary">Open the Tea Party →</a></div>
  </div>
  <div class="docs-card docs-card-rose">
    <div class="docs-heading">♛ The Red Queen's Court</div>
    <div class="docs-copy"><strong>Enforces architecture inside the code repo.</strong> PreToolUse hooks block in milliseconds before any agent tool fires. MCP <code>validate_action</code> lets agents ask the deterministic policy engine before acting. The Red Queen's Court evaluates CALM flow constraints, security-critical paths, restricted-tier locks, and platform impact — and returns <em>allow / conditional / deny</em> on every move. Phase 9 adds a CI hard merge gate.</div>
    <div class="docs-copy"><a href="/docs/red-queens-court" class="docs-button-secondary">Open the Court →</a></div>
  </div>
</div>

The Hatter governs **what you should build and how it should be designed**. The Red Queen governs **what the agent is allowed to do** while it builds. They are complementary, not competing — and they share the Looking Glass substrate so an OKR-anchored design and a runtime enforcement decision both reference the same CALM nodes, the same threat IDs, the same governance scores.

---

## What no one else has

We reviewed the leading vendors across Internal Developer Portals, Enterprise Architecture Management, and AI-Assisted Software Engineering. Each has real strengths. But every one of them is missing the same thing: **architecture governance that reaches the developer and the agent simultaneously.**
...
<div class="docs-grid docs-grid-wide">
  <div class="docs-card docs-card-indigo">
    ...
    <div class="docs-copy">The OKR-driven planning pipeline. End-to-end mockups, the four-oracle research wheel, ask-experts loop, code-design gate, audit export.</div>
    <div class="docs-copy"><a href="/docs/hatters-tea-party" class="docs-button-secondary">Read →</a></div>
  </div>
  <div class="docs-card docs-card-rose">
    <div class="docs-heading">♛ Red Queen's Court</div>
    <div class="docs-copy">Deterministic governance enforcement at the agent's tool-call boundary. Three layers, the six rails today, cross-repo semantic governance, the Phase 9 hard gate.</div>
  ...
  <div class="docs-card docs-card-cyan">
    <div class="docs-heading">Framework + Workshop</div>
    <div class="docs-copy">The 6-phase SDLC framework that ties STRIDE, OWASP, fitness functions, and agent guides together. Plus the 8-part workshop curriculum.</div>
    <div class="docs-copy"><a href="/docs/framework" class="docs-button-secondary">Framework →</a>&nbsp;&nbsp;<a href="/docs/workshop" class="docs-button-secondary">Workshop →</a></div>
  </div>
  <div class="docs-card docs-card-emerald">
    <div class="docs-heading">Red Queen Quickstart</div>
    <div class="docs-copy">Hands-on walkthrough: install hooks, MCP runner, review workflows into a real repo using the IMDB-Celebs BAR.</div>
