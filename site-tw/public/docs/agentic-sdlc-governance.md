...
<div class="docs-grid docs-grid-wide">
  ...
  <div class="docs-card docs-card-rose">
    ...
    <div class="docs-copy">Formal STRIDE extension that adds an "A" category for AI-agent-specific attacks: prompt manipulation, memory poisoning, inter-agent influence. The STRIDE+ASTRIDE coverage SVG above shows the agent-side controls. <code>A.false-audit-fabrication</code> surfaced on a real PR and is closed by four layered defenses: a pre-merge <code>verify-chain</code> CI gate (forged-hash detection), Knight's Seal v1 (per-event, per-epoch Ed25519 signing — every agent event signed under the active session's keypair), <strong>three-class event ownership (Court Recorder Auto-Logging + Bug V/W)</strong> — the runner auto-emits <code>skill_call</code> from inside <code>runSkill()</code>, the workflow re-derives <code>artifact_written</code> / <code>state_transition</code> / <code>human_gate</code> from canonical sources, and the agent emits <code>self_review</code> signed under its per-epoch key from inside the persona-prompt section (signed LLM judgment, not deterministic evidence) — and a <strong>written audit-event contract pinned by a regression test</strong> so the shape itself can't drift. Companion threats <strong>A.audit-skip</strong> (agent omits events to hide failures — caught by the chain-vs-block parity check + sealed=true verdict gate) and <strong>A.audit-forge-payload</strong> (agent fabricates artifact_written or self_review content — artifact_written is re-derived from <code>git diff</code>; self_review is signed under the agent's per-epoch key; the runner's narrow workflow-emittable allowlist <code>{artifact_written, state_transition, human_gate}</code> plus the signed-workflow-event rejection (Bug X) keeps the workflow-attribution path tight enough to block hand-rolled forgeries) are closed by the deterministic-vs-LLM-judgment split + mechanical drift protection. <br><span style="display:inline-block;margin-top:0.5rem;font-size:0.8125rem;opacity:0.75">📄 <a href="https://arxiv.org/pdf/2512.04785">arXiv 2512.04785</a></span></div>
  </div>
</div>
