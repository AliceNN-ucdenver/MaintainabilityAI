...
<svg viewBox="0 0 800 480" xmlns="http://www.w3.org/2000/svg" class="docs-svg" role="img" aria-label="Oraculum — code-grounded architecture review across target repos">
  ...
  <text x="238" y="207" fill="#cbd5e1" font-size="10" font-family="system-ui, sans-serif">movie-api (brownfield)</text>
  ...
  <text x="44" y="312" fill="#e2e8f0" font-size="11" font-family="system-ui, sans-serif">movie-api/routes/auth.ts:42 — JWT verification missing `iss` + `aud` claims.</text>
...
<div class="docs-report-preview-callout">
  <div>
    ...
    <div class="docs-copy">See the OKR closeout the way an auditor would read it: verdict first, then phase rollup, chain ladder, control coverage, the cross-repo implementation chain + Red Queen seal, the Hatter Oracle rails (injection / groundedness / PII), Pocket Watch alignment, and the verifier command.</div>
  </div>
...
<div class="docs-card docs-card-cyan">
  ...
  <div class="docs-copy">
    ...
    <p><strong>Live-run proof:</strong> on the open IMDB-Lite sample, the agents shipped a real OKR <em>end-to-end through implementation</em> — WHY → HOW → WHAT → a cross-repo code fan-out that merged into <code>movie-api</code>. The exported whole-OKR rollup verdict reads <strong><code>✅ PASS — All 3 phases present, runner-verified, and source-atomic</code></strong>. Each planning phase landed signed (every agent event under a per-session Ed25519 key), source-atomic (canonical GitHub on every input the report cites), and threaded through one <code>intent_thread_uuid</code> in the cross-phase ladder. The implementation row carries the merged impl PR, its <code>parent_chain_root</code> + intent-thread cross-checks, and a <strong>Red Queen seal</strong> over the deterministic policy log (49 decisions, 49 allowed, 0 denied). The <strong>Hatter Oracle rails</strong> re-derived the report bytes and replayed each model in CI: injection and PII passed; groundedness recorded advisory (unpinned model — integrity-gated, non-gating on replay). Control mapping traced every PRD-declared security requirement to its OWASP roots and the design citation, and Pocket Watch logged advisory alignment. Outstanding gaps: <em>none</em>. Click the <em>View sample audit report</em> button above to see the full rollup the way an auditor would read it.</p>
