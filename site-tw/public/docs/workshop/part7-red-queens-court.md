...
**Recorded.** Every override the hook honors is recorded in `.redqueen/audit-log.jsonl` with `action: pre_tool_use`, `verdict: allow`, **`override: true`**, **`bypassedRuleId`** (which rule the approval bypassed, e.g. `TIER-003` for a requireApproval Bash, `TIER-002` for a restricted-tier Edit), and **`approvalSource`** (`REDQUEEN_TOOL_APPROVED`, `REDQUEEN_PLAN_APPROVED`, or `toolInput.redqueenApproved`). The audit log is grep-able. *"How many TIER-003 overrides last quarter?"* is one `jq` query (`jq 'select(.payload.override == true and .payload.bypassedRuleId == "TIER-003")' .redqueen/audit-log.jsonl`), not an archaeology session.
