...
| **AU-2** | Event logging | Red Queen writes a JSONL line per PreToolUse decision into <code>.redqueen/audit-log.jsonl</code> (timestamp, rule, requested action). Signed, hash-chained allow / conditional / deny / override evidence is <em>Queen&rsquo;s Next Act</em>. | <code>.redqueen/audit-log.jsonl</code> |
...
| **T8** | Repudiation / untraceability | Hatter side, shipped: <em>The Knight&rsquo;s Seal v1</em> signs each audit event under a per-epoch Ed25519 keypair on the planning-side hash-chained audit log; verifier rejects any unsigned, mis-signed, or out-of-chain event. Red Queen side, today: enforcement decisions land in <code>.redqueen/audit-log.jsonl</code> (plain JSONL). Red Queen Next Act: same three-lane trust contract over allow / conditional / deny / override decisions. | Hatter chain + signature; <code>.redqueen/audit-log.jsonl</code> |
...
| **A.6.2.8** | Logging | Shipped: Hatter hash-chained, Knight&rsquo;s Seal v1 signed audit chain on the planning side; <code>.redqueen/audit-log.jsonl</code> records every Red Queen enforcement decision. Cross-chain SIEM export and inclusion proofs are <em>Queen&rsquo;s Next Act</em>. | Hatter chain; <code>.redqueen/audit-log.jsonl</code> |
...
| **Art. 12** | Automatic logging (model version, inputs, operator, timestamps, &geq; 6 mo) | Shipped: Hatter hash-chained, Knight&rsquo;s Seal v1 signed audit chain + <em>Hatter&rsquo;s Tag</em> per-PR provenance manifest; <code>.redqueen/audit-log.jsonl</code> for every enforcement decision. Cross-chain inclusion proofs and 6-month retention export are <em>Queen&rsquo;s Next Act</em>. | Hatter chain; <code>provenance.json</code>; <code>.redqueen/audit-log.jsonl</code> |
