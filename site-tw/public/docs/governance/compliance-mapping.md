...
| **A.8.27** &mdash; Secure system architecture and engineering principles | Engineering principles for secure systems shall be established | CALM 1.2 architecture model + [Looking Glass](/docs/impossible-things) governance scoring | CALM model in mesh; four-pillar score |
...
| **AU-2** | Event logging | Red Queen audit log of every allow / deny / override | Audit log export from mesh |
...
| **MG-3.2** | Audit trail | <em>The Court Recorder</em> Merkle-chained log <em>(roadmap)</em> | Audit JSONL export |
...
| **A.6.2.8** | Logging | <em>The Court Recorder</em> Merkle-chained audit log <em>(roadmap)</em> | Audit JSONL stream |
...
| **Art. 12** | Automatic logging (model version, inputs, operator, timestamps, &geq; 6 mo) | <em>The Court Recorder</em> (audit) + <em>The Hatter&rsquo;s Tag</em> (per-PR model + prompt fingerprint) <em>(both roadmap)</em> | Audit JSONL; <code>provenance.json</code> per PR |
