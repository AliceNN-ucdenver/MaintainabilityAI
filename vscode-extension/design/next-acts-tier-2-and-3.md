...
- **(e) Agent memory + cross-agent instruction layer — ⬜ DESIGN CAPTURED; not built.** Backs the "agents that learn" + "Agent roster and policy learning" cards in `red-queens-court.md`. Two layers: a portable instruction spine (`AGENTS.md` + Copilot `*.instructions.md` overlay + `CLAUDE.md` `@AGENTS.md` import) and a governed learning loop (distill override / repeated-denial signals → human-reviewed candidates → **signed** promotion into `AGENTS.md` / `*.instructions.md` / `customRules` / prompt packs; mesh vs repo tiers via `.caterpillar`/`.cheshire`; every learning carries provenance + review-state + freshness; memory-poisoning-aware). Phased: E1 roster (read-only) → E2 populate `valueLearning[]` → E3 override reflector → E4 promotion pipeline → E5 spine migration. Full design: [`agent-memory-and-instructions.md`](agent-memory-and-instructions.md).
...
- **Phasing.** (1) **L1 deterministic envelope** (runner TS) over the 4 search skills + dedupe, with fixtures (malicious queries, private URLs, oversized sets) — **✅ SHIPPED** (runner 0.1.63, `withGuardrails` in `SKILLS`; production-proven on a WHY run, quarantined 2/5 unparseable-URL results). (2) **PII rail** (Python/Presidio, tiered policy, on the WHY research-doc + source registry) — **✅ SHIPPED + cert-verified** (ext 0.1.250–0.1.251, OKR-2026Q2-IMDB-002-movie-api: PR-gate PASS · finalize recorded · CI model-replay reproduced `{invoked:true,ok:true}` · rollup re-derived all legs PASS). (3) **Injection rail** (Llama Prompt Guard 2) — adversarial snippet corpus; **this is the T3-6 closure. ✅ SHIPPED + cert-verified + PROMOTED to a required hard gate (ext 0.1.256, 2026-06-01).** Decisions: **(i)** model = **Prompt Guard 2 86M** pinned at HF commit **`a8ded8e697ce7c355e395a0df51f94adb4a2fd27`** (resolved + cert-verified at run 26732315667; HF snapshot in the replay cache key), **(ii)** scan target = **provider snippets / source-registry only** (the true INDIRECT-injection surface — NOT the agent-authored synthesis doc, which is benign security prose), **(iii)** evidence shape = **separate per-rail reports** (`<runId>.rail-report.json` for pii + `<runId>.injection.rail-report.json`), each independently replayable — the single-rail report/replay/exporter triplet is generalized to N rails. **Cert result (5b):** hostile fixture → exit 2 (blocked), benign fixture → exit 0 (clean), `malicious_class=LABEL_1 @idx 1`. **Now enforcing:** `injection.json` has `model_revision` pinned + `require_pinned_revision: true`, so the WHY PR-gate + finalize hard-fail on a blocked/needs_review/not-invoked injection rail; `inject_check.py` fails closed on an unpinned tag or an unrecognized malicious label; replay re-verifies the resolved SHA (a moved tag → `rail-replay-mismatch`). Hard risk (the ORACLE_SECRET false-positive lesson, sharper): the corpus IS security content, so injection-shaped quoted/cited/code-fenced text is *content*, not an attack — diagnostics-first, tune against fixtures, never blind; benign-context carve-out before it gates. (4) **Groundedness rail** — conclusion ⊨ cited source; **🟡 PLANNED, decisions locked 2026-06-01.** This catches what Phase 3's `verify-source-table.mjs` cannot: that gate proves a conclusion *cites* an S[N] anchor AND that the anchor *resolves* to a real (title, url) in the source registry — never that the cited source actually *says* what the conclusion claims. Phase 4 is the semantic layer above provenance. **Architecturally different from rails 2/3:** they scan units independently and skip the agent-authored doc; this is a **pairing rail** — it reads the doc's Formal Conclusions (the hypotheses) and pairs each against the snippet text of its cited S-tags (the premises), reusing the existing S-tag→registry resolver rather than reinventing it. **Decisions:** **(i) model = a small NLI cross-encoder** (DeBERTa MNLI/FEVER/ANLI family, e.g. `MoritzLaurer/DeBERTa-v3-base-mnli-fever-anli`), pinned by HF commit SHA, behind a `GroundednessScorer` interface (`model_family: nli-cross-encoder`) so AlignScore stays a later cert-comparison candidate, not the first gate — premise = cited snippet(s), hypothesis = conclusion sentence, output entailment/neutral/contradiction. **(ii) TIERED policy:** `block_on_contradiction: true` (cited source *refutes* the claim = dangerous citation misuse), `block_on_neutral: false` + `neutral_disposition: needs_review` (source silent/too-narrow, or the conclusion legitimately synthesizes across multiple sources — pairwise NLI marks each premise neutral even when the synthesis is sound; must NOT hard-fail that), entailment ≥ threshold by ≥1 cited source = pass. Rollup phrasing: `contradicted: N` → fail, `unsupported: N` → needs_review/partial, `entailed: N` → pass. **(iii) ship ADVISORY first, then cert→promote** (the noisiest rail — semantic judgment over claim/evidence pairs; deploy advisory, run a fixture corpus (entailed / neutral-unsupported / contradicted) + a well-cited live WHY sample, tune extraction + thresholds, THEN promote *contradiction* to blocking while *neutral* stays needs_review). Reuses the N-rail report/replay/exporter triplet already built — so it is mostly `groundedness.py` + `groundedness.json` + a `groundedness` case in the replay dispatch + MESH_WORKFLOWS entries, far less plumbing than Phase 3. **🔬 LIVE FINDING (2026-06-01, ext 0.1.262 — plumbing healthy, UNIT is wrong):** the first clean advisory run (mechanics fixed: `claims:6` not 11, hypotheses cleaned, combined-premise added) returned **6/6 not-entailed at avg_neutral=0.92, avg_combined_entail=0.06, contradicted=0**. Diagnosis (verified — label indices + premise/hypothesis order correct; not a bug): strict NLI asks "does this premise logically PROVE this hypothesis?" but our Formal Conclusions are MIXED objects — factual evidence + implementation judgment + prescriptive recommendation wording in one sentence ("the endpoint *should* reuse ratings *because*…"). A descriptive evidence snippet ~never *entails* a prescriptive synthesis even when it grounds it, and the combined-premise view did not rescue it (0.06), so it is not thin-single-source. **Nothing is contradicted** — the rail is not calling conclusions false, just not strictly-entailed. **Threshold tuning is rejected:** a threshold low enough to pass this is meaningless. **The real design change is CLAIM DECOMPOSITION, not model/threshold:** the WHY output gains a required `support_claims[]` structure — each Formal Conclusion declares the atomic FACTUAL sub-claims it rests on + the sources for each (e.g. C1 → "AI recs show 10–15% conversion lift [S3]", "CF works on ~10K ratings [S12]", "catalog+ratings drive recs without behavioral PII [S50]"); the rail NLIs each *factual* support_claim against its cited source. The conclusion itself stays a synthesis/judgment NLI is not asked to prove. Until that lands: groundedness stays advisory, the output is honestly labeled "NOT-ENTAILED (strict NLI)" not "unsupported", `score_summary` + per-claim neutral are recorded for observability, and a high-neutral/zero-contradiction run prints an explicit "unit mismatch, nothing refuted" interpretation notice. **Phase 4.1 — claim decomposition via a support-claims sidecar (DECIDED 2026-06-01, plan-of-record; not yet built):** **(a) Contract location = a separate `support-claims.json` sidecar**, NOT a new research-doc section. The research-doc.md stays the human research certificate (its semi-formal narrative is sacred); the sidecar is the machine-readable grounding contract — `okrs/<okr>/why/<runId>.support-claims.json`, shape `{schema_version:"support-claims.v1", okr_id, run_id, phase, claims:[{conclusion_id:"C1", support_claims:[{id:"C1-SC1", text:"<atomic FACTUAL claim>", sources:["S3"]}]}]}`. Hash-pinnable; no implementation-shaped JSON stuffed into prose. **(b) Producer = the WHY agent emits it from the SAME synthesis pass** (not a post-hoc extractor — a second after-the-fact model judgment would itself need governing). **(c) Rollout = advisory-first with graceful fallback:** the groundedness rail PREFERS the sidecar when present (NLIs each atomic factual support_claim against its cited source — the unit NLI can actually judge); when absent it FALLS BACK to today's whole-conclusion strict-NLI path, clearly labeled. The merge gate does NOT block on sidecar absence yet. Sequence: rail reads sidecar (advisory) → prompt update so the agent emits it → cert checks sidecar quality on real runs → only then make the sidecar required for groundedness gating. Rationale (learned twice now — PII + injection both needed live calibration before gating): let the agent emit, let the rail read, let the evidence say when to promote. AlignScore-class support scoring stays a later comparison; decomposition fixes the semantic SHAPE first and is the chosen next move. (5) **Promote to a required gate + a live cert run** — two-part proof, mirror of the T2.5a arc: **(5a) detection corpus** — `python3 -m unittest test_inject_model` runs the REAL Prompt Guard 2 model over a labeled adversarial corpus (`test_inject_model.py`: 6 direct+indirect injection strings that MUST score ≥ block_threshold, 6 benign research strings incl. one that *mentions* injection that MUST score below). Skipped when the model is absent (hermetic suite stays green); GREEN here is the "the rail actually catches bad content" proof. **(5b) live execution** — a WHY run with a hostile injected snippet in the source registry: PR-gate `inject_check.py run` returns exit 2 → blocked, finalize records the injection report, replay reproduces it. Pin `model_revision` to the resolved SHA, flip `require_pinned_revision:true`, run (5a)+(5b) green, THEN the rail is a required gate. (Score-level POLICY tests in `test_inject_check.py` — 79 incl. skips — already prove the decision logic; (5a) proves the model scores real attacks high.) Start with WHY oracle skills; extend to HOW/WHAT + code skills after the contract is stable.
- **Files (as built — Phases 1–2 actual; Phase 3 anticipated).**
  - `packages/research-runner/src/runner/guardrails/envelope.ts` (Phase 1, SHIPPED) — Layer 1 `withGuardrails` + deterministic checks; applied in the `SKILLS` registry for oracle skills + unit fixtures. `rail_decision` added to the runner's emit/origin enums + the `chainVerify.ts` mirror.
  - `vscode-extension/code-templates/workflows/scripts/oracle-rails/` (Python rails — NOT `tools/`; deployed to the mesh `.github/` via `MESH_WORKFLOWS` in `codeRepoTemplates.ts`, fetched from the mesh DEFAULT branch by the PR-gate):
    - `oracle_rail.py` + `pii.json` + `test_oracle_rail.py` + `requirements.txt` (Phase 2, SHIPPED — Presidio, tiered, replay-verified).
    - `inject_check.py` + `injection.json` + `test_inject_check.py` (Phase 3, NEW — Prompt Guard 2 86M; same pure-core + guarded-model-seam shape as `oracle_rail.py`).
    - `groundedness.py` (Phase 4, later).
  - `vscode-extension/code-templates/workflows/market-research-agent.yml` (PR-gate rail step) + `oracle-rails-replay.yml` (CI model-replay; Phase 3 generalizes it to dispatch the right script per the report's `rail` field).
  - `vscode-extension/code-templates/actions/finalize-okr-action/` — generic `rail_decision` recorder (Phase 3: record N rail reports, currently one).
  - `.caterpillar/guardrails/oracle/` — optional mesh-side hash-pinned config override (`pii.json`, `injection.json`); the bundled default ships with the scripts. `config_sha256` in each report pins whichever ran.
  - `AuditReportExporter.ts` — "Oracle rails" rollup subsection, re-derived at export (`computeOracleRailStatus` + `renderOracleRailsSubsection`, folded into `computeOkrRollupVerdict`); Phase 3 generalizes `OracleRailRollupInput` from one rail to a list.
- **Sub-decisions.** *PII policy (Phase 2) — DECIDED:* **tiered** (block hard-fail classes / allow+redact public-figure classes / needs_review on context escalation), not pure refuse — shipped + cert-verified. *Injection model + scan target + evidence shape (Phase 3) — DECIDED 2026-06-01:* Prompt Guard 2 **86M**, **snippets/source-registry only**, **per-rail reports** (see Phasing (3)). *Groundedness model + policy + posture (Phase 4) — DECIDED 2026-06-01:* small **NLI cross-encoder** (DeBERTa MNLI/FEVER/ANLI, behind a `GroundednessScorer` interface; AlignScore = later cert comparison, not the first gate); **tiered** policy (contradiction blocks, neutral → needs_review, entailment passes); ship **advisory first, then cert→promote** (see Phasing (4)). All Phase-4 sub-decisions are now closed — no open Oracle-rail decisions remain.
- **Research sources (2026-05-31).** [Llama Prompt Guard 2 (86M, chosen for Phase 3)](https://huggingface.co/meta-llama/Llama-Prompt-Guard-2-86M) — multilingual direct + indirect prompt-injection classifier, local HF, pinned by commit SHA; the lighter [22M (English-only)](https://huggingface.co/meta-llama/Llama-Prompt-Guard-2-22M) is the fallback if CI cost bites. [Microsoft Presidio](https://microsoft.github.io/presidio/analyzer/) (NER-based PII detection/anonymization, not pure regex — shipped in Phase 2); Phase-4 groundedness model = a small **NLI cross-encoder** (DeBERTa MNLI/FEVER/ANLI family, e.g. [`MoritzLaurer/DeBERTa-v3-base-mnli-fever-anli`](https://huggingface.co/MoritzLaurer/DeBERTa-v3-base-mnli-fever-anli)) chosen 2026-06-01 — premise/hypothesis entailment maps cleanly to the rail and is lighter to replay/pin/explain than [AlignScore (ACL 2023)](https://arxiv.org/abs/2305.16739), which remains a later offline cert-comparison candidate, not the first gate.
