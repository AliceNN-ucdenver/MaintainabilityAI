...
| `signer_epoch` | integer | runner | Which epoch's key signed this event. Original agent invocation = 1; first revise-agent = 2; second revise = 3; … **Present on agent events.** **Absent on workflow events** (`payload.emitted_by = "workflow"`) — those have no signing key and the field is omitted by design. Also absent on legacy chains (pre-Bug-O) — verifiers default missing values to 1 for back-compat. |
