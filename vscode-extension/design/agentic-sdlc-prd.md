...
| `skill_call` audit event emission | — | ✅ Runtime auto-emits inside `runSkill()` — **the agent CANNOT emit this kind via the CLI** (Bug Y restriction: public CLI Zod enum rejects `skill_call` and `llm_call` with `bad-input`; only the internal `emitAuditEvent(..., {internal: true})` path can produce them) |
| `llm_call` audit event emission | — | ✅ Runtime auto-emit ONLY — same CLI restriction as `skill_call` (Bug Y) |
| `artifact_written` audit event emission | — | ✅ Workflow detects via `git diff` and re-derives sha + bytes from the current PR HEAD before emitting; mismatch is treated as a forged event and degrades the verdict (Bug Y closes the pre-emit hole). Re-derivable from canonical source — unsigned, allowlisted. |
| `self_review` audit event emission | ✅ Agent calls `audit-emit-event` from inside the persona-prompt section while the per-epoch private key is in scope. The runner sets `payload.emitted_by: 'agent'` from the `EVENT_KIND_ORIGIN` map (NOT from agent input — Bug Y) and signs the event under the per-epoch key. | — Workflow only cross-checks PR-body block ↔ signed chain event for parity (Bug V/W contract). |
| `gap_loop` audit event emission | ✅ Agent (WHY phase / market-research-agent) calls `audit-emit-event` with `eventKind: 'gap_loop'`. **n/a for HOW phase** — this is the WHY-phase equivalent of the HOW-phase agent's persona-switch semantic markers; the prd-agent has no gap-loop. Pre-Bug-Y this rode on `skill_call` with `payload.skill: 'gap-loop'`; that path is closed because `skill_call` is now runtime-only. The `count-skill-calls` action counts both new + legacy shapes for backward-compat. | — |
| Hash chaining + canonical serialization | — | ✅ Runtime (B25) |
| Per-event, per-epoch Ed25519 signing | — | ✅ Runtime (B27 + Bug O) — signs every runtime + agent event (`skill_call`, `llm_call`, `self_review`, `self_review_exhausted`, `gap_loop`) |
| `state_transition` / `human_gate` events | — | ✅ Workflow (re-derivable from PR labels / reviewer state — unsigned, allowlisted) |

The split is **deterministic-vs-LLM-judgment**, pinned by the `EVENT_KIND_ORIGIN` map (Bug Y / Codex round-9) which assigns every event kind to exactly one of three origin tiers: **runtime** (`skill_call`, `llm_call` — auto-emitted from inside `runSkill()`, CLI-rejected for everyone else), **agent** (`self_review`, `self_review_exhausted`, `gap_loop`, the reserved `review_*` kinds — emitted via the CLI from inside the persona-prompt section, signed under the per-epoch key), and **workflow** (`artifact_written`, `state_transition`, `human_gate` — re-derivable from `git diff` / PR labels / PR reviewer state, unsigned, allowlisted). The runner sets `payload.emitted_by` from the map regardless of what the caller supplied, and the verifier rejects events whose `event_kind` and `emitted_by` disagree with `origin-kind-mismatch-line-N`. The signed chain event is what makes the agent's LLM-judgment events auditable: every `self_review` line carries the persona's per-epoch signature, the cross-checked PR-body block proves the human-readable surface matches, and the runner's narrowed workflow-emittable allowlist `{artifact_written, state_transition, human_gate}` (Bug V) + signed-workflow-event rejection (Bug X) + Bug Y kind→origin enforcement keeps the attribution path tight enough that an attacker can't fabricate a self_review by pretending to be the workflow or pre-emit a forged artifact_written. This closes T10 (agent skips emission to hide failures — caught by chain-vs-block parity check + runner sealed=true gate) and T11 (agent fabricates self_review or artifact_written content — self_review is signed under the agent's per-epoch key, artifact_written is re-derived from git diff + sha-compared at workflow emit time).
...
- ✅ **Knight's Seal v1** (B27 + Bug O / per-event, per-epoch) — every event signed Ed25519 under the active epoch's ephemeral keypair. Original agent invocation = epoch 1; first revise-agent = epoch 2; second revise = epoch 3; etc. Private key in `os.tmpdir()` (NEVER in mesh — verified zero `.priv.pem` files in tree). Public key persisted per-epoch to `<mesh>/okrs/<id>/audit/keys/<runId>.epoch-N.pub.pem` and committed with the PR; the first event of each new epoch also carries the PEM inline as `public_key` so a chain replay can recover the verification key without the file fallback. Looking Glass HOW card displays 🛡 Sealed badge inline with `chain_root: 387494d4f70e…`.
...
**Implications for WHAT phase.** Bug V closed the WHAT reviewer-dispatch decision: the code-design-agent uses the same persona-switch model, but its Architect and Security personas must ground on actual indexed code, explicit `knowledge-code-read` file reads, and cited paths. The retired Tweedle reviewer-agent idea survives only as contrarian logic inside the author agent: switch hats, argue against the draft, sign the judgment, and let deterministic workflow gates verify the evidence. Separate reviewer agents are not part of the current shipped path.
