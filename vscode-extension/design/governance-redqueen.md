...
**Version:** 0.13.3 — Per-Decision Audit Logging + customRules Walker
**Date:** May 24, 2026
...
> **Status:** Phase 8 Complete (MCP package, scaffold, hooks, review workflow). v0.13.3 adds per-decision hook audit logging + customRules walker so the Layer 1 hook now produces durable evidence under the same `.redqueen/audit-log.jsonl` `score_snapshot` already writes to. **Queen's Next Act** (formerly Phase 9): adopt the Hatter trust contract (runtime / workflow / agent kind→origin map, verifier-rejected mismatches, Looking Glass mirrors verifier) over the per-decision audit lines, plus the `redqueen-action` standalone hard gate (AST semantic diff + contract diffs + import/layer graph), full break-glass UX (scoped quarterly budgets, written reasons captured at override time, signed override events, CODEOWNER co-sign, escalating friction, follow-up SLAs), CloudEvents export, SIEM integrations, cross-chain inclusion proofs unifying Hatter planning chain ↔ Red Queen enforcement chain.
...
|---------|------|-------------------|---------|
| 0.13.3 | 2026-05-24 | 7, 16, 17 | Per-decision audit logging in the Layer 1 hook + customRules walker. Closes the long-standing truth gap where only `score_snapshot` appended to `.redqueen/audit-log.jsonl`. Three connected changes: (1) `StaticPolicy` schema extended with `customRules: CustomRule[]` (team-extension point, defaults to []) and `auditLog: { enabled, path }` (defaults on); (2) `generateValidateToolJs` walks `customRules` on every Edit / Write / Bash (regex deny against new_string / content / command when appliesTo glob matches; fail-soft on pathological regex), emits per-decision JSONL audit line in `emitDecision()` with `{timestamp, action: 'pre_tool_use', barId, barName, payload: {tier, agent, tool, filePath, verdict, reason, ruleId, sessionId}}`, and every built-in decision now carries a stable `ruleId` (TIER-001/002/003, CTRL-001, SEC-001, CALM-004); (3) `validate_action` MCP tool appends its own audit entry per call. All writes are line-atomic on POSIX (single line, single write, under PIPE_BUF) and fail-soft (audit failures never crash the hook). Three new tests in `mesh-reader.test.ts`: schema defaults, hook spawn writes audit lines, customRule SEC-101 end-to-end with appliesTo scope check. 583 tests passing. **Queen's Next Act** (formerly Phase 9) reframed: adopt the Hatter trust contract (runtime / workflow / agent kind→origin map, verifier rejects mismatches, Looking Glass mirrors verifier) over the per-decision audit lines, plus signed override events with engineer identity + written reason captured at override time. The per-decision JSONL evidence shipping in this version is the foundation that signing extends. |
