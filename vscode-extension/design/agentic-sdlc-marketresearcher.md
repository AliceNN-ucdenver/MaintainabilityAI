...
- ✅ **Knight's Seal** (B27 + Bug O — landed) — every event signed Ed25519 with per-event, per-epoch ephemeral keypairs (each agent session = its own signer epoch; revise rounds advance to epoch 2, 3, …). Looking Glass WHY card displays 🛡 Sealed badge inline with `chain_root: 152d1aca7d36…`. View Tag + Verify Chain buttons live. CI invokes the runner's `audit-verify-chain` skill per PR — same code path the runner uses to write the chain.
