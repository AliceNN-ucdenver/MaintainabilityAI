...
- **Phase B** — agent path runnable end-to-end on Supervised + Restricted tiers. WHY merged cleanly (PR #103). **Tier 1 + Tier 2 hardening landed 2026-05-21** in this branch (research-runner 0.1.26 + extension build). **B28a (Court Recorder Auto-Logging Tier 1) landed 2026-05-21** in research-runner 0.1.27 — `runSkill()` now auto-emits `skill_call` events when session-context env vars are set. Backward-compat preserved; 7 new tests; 211 runner tests pass. B28b-e tracked separately.
  ...
  - **A12 shipped** — OKR detail target-repos section: per-repo Connect Repo button (opens GitHub Settings → Actions URL) + Mark connected toggle. `targetCodeRepoStatus` persisted via `OKRService.update`. Phase D fan-out will gate on `status === 'connected'`.
...
- HOW (PR #105) — passed audit but exposed the `A.false-audit-fabrication` threat (forged chain). Closed in B25 below. Re-run pending against the post-Tier-1 build to validate honest skill_call counts (`c_arch`/`c_sec`/`c_qual` columns) and a green `🛡 Sealed` badge on both phase cards.

**True remaining work before "Phases A + B complete":**
1. **Validation gate** — fresh end-to-end on the IMDB-Celebs sample after the Tier 1 + 2 build deploys (~30 min): create / re-run the celeb-api OKR, click Start Why → confirm `🛡 Sealed` badge + clean chain, merge WHY PR, click Start How → confirm context-* skill_calls are nonzero + `🛡 Sealed` badge + structural correctness, merge HOW PR, confirm chain-ladder shows WHY→HOW lineage. This is the "first two agents complete" handshake.
