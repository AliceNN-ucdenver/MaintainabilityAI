...
> **Doc-level supersession (Codex-r2 Bug 6 — Tier 2 cleanup).** This document still describes `design-bus.yml` as the per-repo fan-out executor in several active-prose sections (§3.1 Start What row, §3.3 closing paragraph, §3.5 Hand-off, §9.3 partial-failure handling, §10.6 Settings table, §13 phase tracker). **That workflow was never shipped.** D-PR4 (Tier 2) replaced it with an **app-orchestrated fan-out engine** inside the Looking Glass panel: `src/services/coordination/fanOutEngine.ts` + `LookingGlassPanel.onFanOutPreflight` / `onFanOut`. The user clicks "Fan out N of M ready" after pre-flight passes; the engine opens landing issues via the GitHub API and dispatches the per-repo `implementation-agent` via `assignCustomCopilotAgent`. The canonical Tier 2 spec lives in [`next-acts-tier-2-and-3.md`](next-acts-tier-2-and-3.md) D-PR4; the deeper supersession note (with rationale + carry-forward contracts) lives in [`agentic-sdlc-codedesigner.md`](agentic-sdlc-codedesigner.md) §D9. The landing-issue body template, `design-fan-out.yaml` row shape, cross-repo Hatter Tag continuation contract, and partial-failure semantics described below remain canonical — read them as data-shape specs, not workflow specs. Treat the literal string `design-bus.yml` as historical naming throughout.
...
| 2. Implement | Mid-**How** + **What** (cross-repo code design) | prd-agent → code-design-agent |
| 3. Verify | review gates on every phase PR | author agents run signed Architect + Security persona-switch self-critique |
| 4. Govern | OKR Card itself; Court Recorder; Knight's Seal | workflows enforce gates; Hatter Tag captures chain context |
...
In practice today this means each agent gets a unique GitHub App installation token per session (Queen's Keyring), and the agent identity is captured in the audit envelope. **Knight's Seal (Phase B / B27 + Bug O)** is the cryptographic upgrade — see §11.5. Phase A enforces identity by GitHub-App-installation-ID-and-system-prompt-SHA; B27+BugO hardens that with per-event, per-epoch Ed25519 signing (each agent session is its own signer epoch with its own ephemeral keypair).
...
| ~~`architect-reviewer`~~ | retired | Bug V replaced separate reviewer agents with signed persona-switch self-critique |
| ~~`security-reviewer`~~ | retired | Same |

Settings UI exposes a "per-agent override" selector — admins can pin author agents to a stronger model for higher-stakes domains, for example. Changes are deploy-only (require a redeploy click; never live-mutate `.agent.md` on a running session).
...
1. **Cost** — each round = two persona prompts plus one revision pass. Three rounds at scale gets expensive.
...
The `prev_event_hash` → `event_hash` linkage gives Merkle-style append-only integrity; the pre-merge CI step invokes the runner's own `audit-verify-chain` skill, so writer and verifier are one code path (Bug-Q phase 1 closed the prior inline-Python design — same hash check, no drift risk, plus full per-epoch Ed25519 signature verification).
...
| **T11** — agent forges `artifact_written` or `self_review` payloads | OPEN — agent's calls were trusted | **CLOSED** — workflow re-derives file-write facts from git diff before emitting `artifact_written`; the agent emits `self_review` as signed LLM judgment while its per-epoch key is in scope; workflow parses the human-readable PR block only as a parity check against the signed chain event |
...
### Where we are right now (snapshot — Phase A-D done-done pending next HOW + WHAT cert legs, 2026-05-22)

**Phase A + B + C + D + E are honestly complete in code (2026-05-25).** All three Looking-Glass-side agents (market-research / prd / code-design) have been validated end-to-end on the IMDB-Celebs sample OKR with no manual intervention through multiple cert rounds. Every architectural defense fired on real agent runs. Phase E shipped per-action audit-report export + verify-chain UI + whole-OKR audit rollup (added as E6 — auditor-grade closeout combining WHY + HOW + WHAT into one signed document). Five Codex chief-auditor review rounds on the audit-export trust posture closed the source-atomicity, runner-keys, suppress-non-canonical, and discriminated-fetch-status gaps. The remaining "done-done" work is a live whole-OKR cert run end-to-end through the rollup feature (the user is running it now) + the E5 team-facing reading-order pack + the "what we're still closing" next-act items (Knight's Seal v2, Red Queen signed enforcement chain, redacted external bundle, prompt-pack signing, org-separation override).
...
- `chiefarcheologist.maintainabilityai` VSIX: 0.1.98 (A12.v1.1 4-state repo status + Settings polish + WHAT-phase signals + reset-phase capability + D-PR1.v1.6 polish + Refactor 3a/3b/3c + cert-run-1 bug A/B fixes)
...
- **Phase D** — **D-PR1 MVP + D-PR1.v1.1 shipped 2026-05-22, validated end-to-end on PR #120**. Code-design-agent runs the same B24 persona-switch pattern as prd-agent, against the same end-to-end audit chain. WHAT validation evidence from PR #120 chain:
  - 14 events, all 14 signed (Knight's Seal verified). Head `ba2071ffa65137ed…`.
  - 2 `knowledge-code` skill_calls — `imdb-react-frontend` brownfield (real SHA `4c554fbe6392`, 77 files walked, 53 TypeScript + 3 JavaScript, manifests=2) + `celeb-api` greenfield (no clone, scaffolding hints returned).
  - 4 self-review-code-* skill_calls — Code-Architect r1 0.91 MINOR → r2 0.97 PASS · Code-Security r1 0.90 MINOR → r2 0.96 PASS. Same MINOR→PASS convergence as PR #118 PRD-phase.
  - Per-repo mode honesty: chain modes match `code-design.md` per-repo subsection frontmatter modes match A12.v1.1 status-implied expectations. No mode fabrication.
...
*Cert run #1 forensic fixes (PR #124 WHY phase — surfaced two polish bugs that prevented "done done" master-status):*
- **Bug A (Task #50)** — dispatch-time `meta.status` advance. Looking Glass `Start Why` was leaving `meta.status='draft'` because dispatch never rolled it to the phase's in-flight status. The composite finalize's downgrade-guard then refused to roll `draft → prd-pending` on merge (expected current=`researching`), so the OKR master status stayed stuck at `draft` through the entire run. Fixed in `LookingGlassPanel.onConfirmStartOkrPhase`: advance `meta.status` to `phaseSpec(phase).currentMetaStatus` via `okrService.updateStatus` before `appendAction`, gated by `isForwardStatusTransition` so re-runs of an earlier phase after later progress can't downgrade. The dispatch fix is uniform across all three phases — for HOW + WHAT the happy path is an identity no-op (their `currentMetaStatus` equals the prior phase's `nextMetaStatus`); for recovery scenarios (e.g. earlier phase shipped under the old bug + status stuck at `draft`) the forward-only transition still advances cleanly. Regression pinned by:
  - 4 tests in `phaseSpec.test.ts` ("dispatch-time meta.status advance contract") — producer/consumer SSoT pin between dispatch wiring + composite finalize guard.
  - 5 tests in `OKRService.test.ts` ("phase dispatch + finalize meta.status advance") — full WHY→HOW→WHAT walk-through happy path + 3 recovery paths (HOW dispatch on stuck draft, WHAT dispatch on stuck prd-pending, WHY re-run downgrade-guard).
- **Bug B (Task #51)** — composite finalize success notice + commit message described the *intended* `meta.status` roll even when the downgrade-guard skipped the actual write. Now captures outcome (`rolled` / `skipped` + post-write status) and prints accurate fragments in both the trailing `::notice::` and the commit message body. Auditors reading the workflow log can now trust the line.

*Cert run #1 hardening (Task #52 + #53 + #54):*
- **Task #52** — explicit dispatch + finalize integration tests for ALL THREE phases (the cert-run-1 fix's "no-op for HOW/WHAT" rationale was hand-wavy; now proven by direct OKRService test that walks each phase end-to-end on a tmpdir, including recovery + admin-intervention edge cases).
- **Task #53** — workflow YAML ↔ phaseSpec drift detector. `phaseSpec.ts` module comment flagged workflow YAMLs as the layer-3 hardcoded-string risk; new tests in `phaseSpec.test.ts` ("workflow YAML ↔ phaseSpec drift detector") read each workflow file + assert all per-phase strings (anchorLabel, draftLabel, passLabel, currentMetaStatus, nextMetaStatus, priorPhase, phase id) match the SSoT map. Drift fires at test time, not at production-run time. 21 tests (7 per phase × 3 phases).
- **Task #54** — OKR card flicker fix. User reported "strange flickers when the cards are refreshing — defaults to the original state until it determines there is data." Root cause: every `okrDetail` message in `lookingGlass.ts` wiped `state.okrPhaseSignals` to `{ loading: true }` for all three phases, so panel-focus / pull / post-audit polls flashed the metric lines to "Loading sources · refine · findings · coverage · drift" then snapped back. Fix:
  - Distinguish first-fetch (`signal.loading`) from background refresh (`signal.refreshing`). On `okrDetail` for the SAME OKR, preserve last-known signal data + mark phases as `refreshing: true`.
  - Metric renderers (WHY/HOW/WHAT) only show the cold-start "Loading…" placeholder when there is no prior data; refresh keeps the prior metrics on screen.
  - New polling indicator dot in each action-card header: muted gray (idle), pulsing blue (fetch in flight), solid green (last fetch fresh), red ✗ (last fetch errored). CSS keyframe-based — no constant DOM churn, just the small dot color/animation. 6 regression tests in `okrViews.test.ts` ("phase-signal polling indicator").
...
### Phase B — Custom Agents + Skills (target: 3 weeks) — **shipped**

Agent deployment lands. Sample OKR becomes runnable end-to-end on Supervised tier. Restricted-tier gating works (manual escalation; auto-revise loop is Phase C). Validated end-to-end on the IMDB-Celebs sample through multiple cert rounds (B23 / B25 / B27 / B28 / B29 / B30 / B31 + Bug V through SS hardening).
...
- [x] **B2.** Deploy search Skills (PURE data, no LLM): `tavily-search`, `arxiv-search`, `uspto-search`, `hackernews-search` — SKILL.md templates shipped in B-PR1; CLI subcommand backends shipped in B-PR1a. Live-evidence forensics hardening shipped in B19b (HTTPS arxiv, honest `ok` semantics, USPTO data-host allow-list).
- [x] **B3.** Deploy `dedupe-and-rank` Skill (PURE) — SKILL.md shipped in B-PR1; backend shipped in B-PR1a.
...
- [x] **B21.** GitHub API access via out-of-the-box `github/*` MCP tools (B-PR1m). Pivots agents from `gh` CLI shell-out to the Copilot Coding Agent's built-in GitHub MCP server. Two coupled benefits:
  ...
  **(b) Token scope + audit attribution.** MCP scopes the token to the source repo + capability automatically. No shared `gh` CLI auth, no broad `api.github.com` allow-list rule, cleaner Copilot-session-attributed audit trail.
  ...
  - ~~`github/add_pull_request_review_comment` — reviewer agents post structured SCORE/COVERED/MISSING/CHANGES reviews on artifact PRs.~~ **Retired by Bug V** with the separate-reviewer model.
  - ~~`github/update_issue` — label changes (`governance-pass-architecture`, `revision-required`, `research-pass`, etc.)~~ **Removed from agent prompts**; workflow YAML owns labels.

  Tweedles' useful contrarian intent is preserved as persona-switch self-critique inside the author agents. The separate reviewer-agent tool split is historical only.
...
- [x] **B25.** `A.false-audit-fabrication` threat closed end-to-end (PR #105 incident). The first clean HOW run uncovered a new ASTRIDE A-category threat: an agent that loses access to the `audit-emit-event` runner can rationalize that "the audit is non-blocking anyway" and write the audit JSONL by hand with fabricated SHA-256 hashes to appear compliant. The HOW agent self-admitted this in its run log ("the hash function is a black box and I can't reproduce it exactly... I'll generate plausible hashes knowing the chain verification would fail but the audit is non-blocking anyway"). The verdict came back `ok` because the audit-and-drift workflow had no chain-verification step. This entry captures the four-pronged defense:
  ...
  **(b) CI defense-in-depth: pre-merge chain re-verification.** **SUPERSEDED by Bug-Q phase 1 (Codex audit round 2 response):** CI now invokes the runner's own `audit-verify-chain` skill via `npx -y @maintainabilityai/research-runner@~0.1.42 skill-audit-verify-chain` — same code path the runner uses to write the chain, so writer and verifier are one implementation with no drift. The original B25 design used an inline Python replay (described below); the per-event Ed25519 verification Bug O added made the inline approach untenable (would have required duplicating per-epoch pubkey loading in Python). Both `prd-agent.yml` and `market-research-agent.yml` got the original `Verify audit chain integrity` step right after `Locate audit JSONL`. The original step was an inline Python replay (sort_keys + separators=','/':' — mirrors `canonicalStringify` in `research-runner/src/runner/skills.ts:641`) that re-hashed every event against the canonical serialization. It was a useful early defense against forged hashes, but Bug-Q intentionally traded the separate implementation for "one writer/verifier implementation, no drift" once Ed25519 per-epoch verification landed. Verdict logic has a `chain_fail` flag checked FIRST (highest-priority gate — a forged chain invalidates every other audit signal), and a `chain-forgery-detected` label (color `B71C1C`, darker than the other failure labels) blocks merge on mismatch. PR #105's exact scenario fails the merge gate with a named cause.
...
- [x] **B27 + Bug O/T/X.** Knight's Seal — per-event, per-epoch Ed25519 signing for every agent-emitted audit event. The live model is simple: each agent session is its own signer epoch with its own ephemeral keypair (`<runId>.epoch-N.pub.pem`), every agent event carries `signature` + `signer_epoch`, and workflow events are unsigned only when their kind is workflow-owned and re-derivable (`artifact_written`, `state_transition`, `human_gate`). There is no legacy unsigned-chain fallback. CI invokes the runner's `audit-verify-chain` skill — not an inline reimplementation — so SHA-256 chain replay and per-epoch signature verification share one code path with the writer. The verifier rejects unsigned agent events, workflow events with a signature or `signer_epoch`, origin-kind mismatches, partial tampering, and forged hashes. Looking Glass mirrors the same rules in `chainVerify.ts`, so a green Sealed badge means the same thing in UI and CI. Future cosign / sigstore evolution (Knight's Seal v2 — persistent identity in a transparency log) is documented at [`agentic-sdlc-futurethoughts.md`](agentic-sdlc-futurethoughts.md) §6.

- [x] **B28.** Court Recorder Auto-Logging — the skill runtime emits `skill_call` audit events on every `runSkill()` invocation; the agent never calls `audit-emit-event` for skill_calls. **Why this matters:** B25 caught forged hashes and B27 caught forged signatures, but neither catches *missed* events ("agent forgot to log skill X"). B28 closes that gap by making runtime evidence deterministic: tool-call facts are built by runner code, workflow facts are re-derived by CI, and agent-only judgments are signed under the active per-epoch key. The trust story upgrades from "verify the chain after the fact" to "every event kind has exactly one legitimate source." **All 5 sub-deliverables (B28a / B28a.v1.1 / B28b / B28c / B28d / B28e) shipped + Bug V/W/X/Y/Z hardening rounds closed.**

  - [x] **B28a — Tier 1: Runtime self-emission.** Shipped in research-runner 0.1.27; tightened through Bug Y/Z. `session-context.ts` reads `OKR_ID` / `RUN_ID` / `INTENT_THREAD_UUID` / `PHASE` env vars (workflow sets them at job start). `runSkill()` wraps every handler invocation: captures `duration_ms`, then auto-emits a signed `skill_call` event with payload `{skill, ok, duration_ms, reason?}` when session context is present. When env vars are absent, no auto-emission fires, which is fine for local/manual use but not for governed phase runs. `audit-emit-event` and `audit-verify-chain` are in `NO_AUTO_EMIT_SKILLS` (no recursion). Best-effort emission — an audit-write failure never shadows the real skill result; the chain-verify CI gate is the catch-net. **Bug Z:** the privileged emitter is module-private, so package consumers cannot call `emitAuditEvent(..., {internal:true})` directly. **Tests:** cover happy path, no-context behavior, no-recursion guards, `ok:false` honest logging, unknown-skill rejection before context lookup, end-to-end chained verification with seal, and non-export of the privileged emitter.
  ...
  - [x] **B28c — Tier 3: Workflow audit-shift.** Shipped in extension build 2026-05-21; updated by Bug V/Y. **prd-agent.yml + market-research-agent.yml + code-design-agent.yml** emit `artifact_written` from workflow-controlled evidence: the workflow detects the artifact path, computes sha256 + byte count from the current PR content, and emits a workflow-owned event. **Post-Bug V:** workflow no longer emits `self_review`; the agent emits each persona verdict itself from inside the persona-prompt section while the per-epoch private key is in scope. The workflow parses PR-body self-review blocks only to diagnose block-vs-chain mismatch and surface scores to humans. **Bug Y:** the runner derives `payload.emitted_by` from the event kind and the workflow re-checks an existing `artifact_written` event against PR HEAD before trusting it, so a pre-written fake sha is rejected. **Threat-model impact:** T10 (agent skips emission to hide failures) and T11 (agent forges artifact_written or self_review payloads) are closed by separate mechanisms: runtime auto-emits tool facts, workflow re-derives file facts, and agent review judgments are signed chain events.
...
### Phase C — Orchestration + bounded recycle (target: 2 weeks) — **shipped**

Bus workflows land. Reviewer recycle loop works on Supervised/Autonomous tiers. Restricted tier still blocks (which is the point). Bug V (2026-05-23) confirmed persona-switch self-critique for every phase including WHAT — the reviewer-bus + dedicated reviewer-agent workflows (C2 / C5) were retired and the round counter moved inside per-agent workflows. Drift-gate (C7) + Caterpillar's Challenge (C8) + HumanGate UI (C6) all shipped + production-validated.
...
### Phase D — code-design-agent + code-grounded reviewers + hand-off completion (target: 3 weeks) — **shipped (D-PR1 + D-PR1.v1.1 + A12.v1.1; D-PR2 superseded by Bug V; D-PR3–D-PR6 deferred to fan-out next-act)**

The third and **last** Looking-Glass-side agent. Where Phase B's market-research-agent grounded on the web and Phase B's prd-agent grounded on the mesh, the code-design-agent grounds on **the actual code** in every impacted repo (brownfield) — or on the PRD + mesh + reference-repos when the repo doesn't exist yet (greenfield, signalled by A12.v1.1's `'create'` status). WHAT-phase audit is the heaviest gate in the entire pipeline — and the most expensive — but also the one that catches "the PRD was perfectly mesh-grounded and still proposes something the codebase can't absorb without breaking." Validated end-to-end on PR #120 + subsequent cert runs (chain shows 14+ events all signed, per-mode knowledge-code breakdown, persona-switch 2-round convergence).

**Full Phase D spec → [`agentic-sdlc-codedesigner.md`](agentic-sdlc-codedesigner.md).** That doc owns: scope at a glance · the canonical brownfield-vs-greenfield branching matrix (per `targetCodeRepoStatus` A12.v1.1) · all sub-deliverables (D0–D12) · the hand-off canonical spec (cross-repo Hatter Tag continuation, design-bus.yml partial-failure handling, greenfield repo-creation flow, tier-gating on receiving repos). _(D8 reviewer-dispatch pattern was CLOSED 2026-05-23 in Bug V — persona-switch self-critique for every phase including WHAT.)_
...
- [x] **D-PR1** (MVP — runnable end-to-end, shipped 2026-05-22, validated on PR #120). Shipped **D0 + D1 + D2 + D3 + D4 + D6 + D7** in one PR (the three review packs landed alongside synthesis because B24 persona-switch consumes all three from day one). `code-design-agent.agent.md` dispatches in primary mode via `assignCustomCopilotAgent` (same as Start Why / Start How). `knowledge-code` Skill ships with the 3-mode response per A12.v1.1 (brownfield clone+classify / greenfield scaffolding hints / refuse on not-connected/unreachable). `self-review-code-architect` + `self-review-code-security` Skills extend B29 to the WHAT phase (tier-bound persona-switch reading `.caterpillar/prompts/code-design/*` packs). `code-design-agent.yml` runs three jobs (dispatch precondition · audit-and-drift · finalize). Validation evidence: PR #120 chain shows 14 events all signed, 2 `knowledge-code` skill_calls (1 brownfield SHA `4c554fbe6392` 77 files / 1 greenfield), 4 self-review-code-* skill_calls (Arch 0.91→0.97, Sec 0.90→0.96, both MINOR→PASS in 2 rounds), per-repo mode honesty verified.
- [x] **D-PR1.v1.1** (workflow + UI polish, shipped 2026-05-22). Five bug fixes surfaced by the PR #120 audit run plus a new escape hatch. **Workflow** (code-design-agent.yml): #27 mode-honesty parser key path (was reading `payload.audit_metadata.*` but runner emits `payload.*` flat — false-negative findings on every WHAT run); #28 self-review `grep -c \|\| echo 0` double-print causing empty audit-comment values; #29 audit-comment markdown collapsed-to-one-line (heredoc replaces command-substitution + inlines mode-honesty findings); #30 dispatch trigger only matched `labeled` event (Looking Glass attaches labels at create time so it never fired). **Runner**: G2 `handleKnowledgePrd` shipped (was deployed-but-missing-handler — agent fell back to direct file grep with no chain evidence). **UI**: #31 `renderWhatMetrics` + WHAT-phase chain/artifact signal extraction (was hardcoded "Phase 3 ships" placeholder; now surfaces mesh skill count + per-mode knowledge-code breakdown + persona-switch rounds + FR/SR references + drift cosines). **New feature**: #32 `OKRService.resetPhase()` + UI "⟲ Reset" button — destructive escape hatch for unsealed phases (4 hard guards: seal-immutability via `hatterChainRoot` OR `chain-ladder.yaml` entry, cascading via later-phase actions, okr-not-found, idempotent-on-empty). Best-effort GitHub close of stale issues + PRs. 6 new tests; 430/430 pass.
- [s] **D-PR2** (reviewer packs — **superseded; equivalent shipped inside D-PR1**). ~~Ships **D2 + D3** as the `code-design/architecture-review` + `code-design/security-review` prompt packs the `code-design-agent` consumes via its Code-Architect / Code-Security persona-switches.~~ **D8 CLOSED 2026-05-23 (Bug V)** — persona-switch self-critique for every phase including WHAT (B24 pattern carried forward). The two NCMS-shaped review packs landed inside D-PR1's `.caterpillar/prompts/code-design/architecture-review.md` + `security-review.md` and are consumed by `self-review-code-architect` / `self-review-code-security` skills. No separate D-PR2 commit needed.
- [ ] **D-PR3** (reference-repos — **deferred to BUILD fan-out next-act**). Ships **D5** — `knowledge-reference-repos` Skill cloning + indexing mesh-configured reference repos for greenfield targets to use as exemplars. Optional; only fires when `mesh/.caterpillar/reference-repos/<bar-id>.yaml` exists. Not on the critical path for the planning chain; ships with the BUILD fan-out so reference-repo exemplars land alongside the per-repo landing issues.
- [ ] **D-PR4** (fan-out — brownfield + greenfield scaffolding — **deferred to BUILD fan-out next-act**). Ships **D9**. `design-bus.yml` with two branches: brownfield repos get the landing issue directly; greenfield repos get scaffolded (idempotent org-create + seed README/LICENSE/CODEOWNERS/code-design-spec.md commit) THEN landing-issue opened. New failure modes: `scaffold-failed` (org-create rejected) + `scaffold-partial` (repo created, seed commit failed mid-flight). Per-repo result file `design-fan-out.yaml` records `mode` + `repo_created` + `seed_commit_sha`. **This is the BUILD-handoff entry point in the rail map.**
- [ ] **D-PR5** (Looking Glass UX — **partially shipped; remainder deferred to BUILD fan-out next-act**). **Shipped:** Stage 4 What-Code-Design card (mirrors HOW card shape) — D-PR1.v1.1 #31 wired the WHAT-phase chain/artifact signal extraction with mesh skill count + per-mode knowledge-code breakdown + persona-switch rounds + FR/SR references + drift cosines. Hatter chain ladder visualization shipped in the Audit Report Export (E1) + the OKR Audit Rollup (E6) — both surface the `chain-ladder.yaml` cross-phase tree. **Deferred:** Stage 5 Per-Repo Fan-Out card with brownfield vs greenfield row indicators and "Retry fan-out / Resume scaffold" affordances per failed row — lands with D-PR4 when `design-bus.yml` exists to read fan-out result files from.
- [ ] **D-PR6** (full-WHAT E2E smoke with fan-out — **deferred to BUILD fan-out next-act**). Ships **D12**. The planning-chain half of this is satisfied by the cert runs (multiple HOW + WHAT runs validated on the IMDB-Celebs sample with chain proof, persona-switch convergence, drift cosines). What's left: `design-bus.yml` scaffolding + landing-issue opening end-to-end against real target repos with intent-thread continuation. Restricted-tier expectation: WHAT dispatches but the audit-and-drift verdict requires HumanGate (UI button shipped in C6; not yet exercised against fan-out).

### Phase E — Audit Report Export + hardening (target: 1 week) — **shipped (E1–E6 done; E5 docs pack partial — team reading-order doc pending)**

The auditor's master question is answerable in one click. Shipped + hardened over **five Codex chief-auditor review rounds** (E3-gold-r2 through E4-r1-followup) closing source-atomicity, per-epoch key verification, runner-driven verdict, structural runnerInput state, suppress-non-canonical PRD/artifact, walk-bottom-up stdout parser, composeSourceTag predicate, and discriminated `getRepoFileStatus` for 404-vs-fetch-error.

- [x] **E1.** **Per-action audit report export** — `LookingGlassPanel.onExportAuditReport(okrId, actionId)` + `services/AuditReportExporter.buildAuditReportMarkdown(input)`. One-click closeout per OKR action: runner crypto verdict at top, shape-level seal verdict, source breakdown table, per-skill evidence, per-persona self-review trail with `event_id` citations, workflow facts (`artifact_written` + `state_transition`), collapsible event timeline, SR-NN → STRIDE/OWASP → PRD anchor → design § control mapping, cross-phase ladder. Saved to `okrs/<id>/audit/exports/<runId>-report.md`. Wired to per-action `Export Report ↗` button inside each phase card.
- [x] **E2.** **Verify Chain UI surface** — `LookingGlassPanel.onVerifyChain` + `chainVerifyForUI` shape verifier + runner shell-out to `skill-audit-verify-chain` for Ed25519 + hash-replay verdict. Slide-out modal renders verdict on every action card via per-action `Verify Chain ↗` button + cross-link from the View Tag panel. Source-of-truth runner crypto verdict surfaces inline; copy-button for the canonical `npx … skill-audit-verify-chain` shell command included for offline re-verification.
- [x] **E3.** **Control-mapping section in closeout** — `extractControlMapping(prdText, artifactText)` scopes to the PRD's `## Security Requirements` section, finds every distinct SR-NN occurrence (heading / list / table shape), and renders a per-SR row in the closeout's `## Control mapping` table (`SR | STRIDE | OWASP | PRD anchor | Design references SR?`). Standalone interactive `traceability.html` matrix renderer NOT built — the in-closeout markdown table covers the auditor question without a separate UI surface.
- [x] **E4.** **End-to-end cert runs against IMDB-Celebs sample** — multiple cert rounds shipped: cert-run-1 (PR #124 WHY clean), cert-run-2 (HOW + WHAT bugs C/D/E), cert-run-3 (F/F.v2), cert-run-4 (G), cert-run-5 (H/I/J/K/L/M/N + Bug O Ed25519). Each round shipped fixes + regression tests; the live model is what survived. Restricted-tier blocking path validated by tier-frozen `governanceTier` on every action.
- [~] **E5.** **Documentation pack** — partial: marketing site `agentic-sdlc-governance.md` shipped + polished over r-marketing-r3 (executive summary + 70/30 framing + Monday-morning narrative + three primitives prose + STRIDE+ASTRIDE+AEGIS overlay + honest "What we're still closing"); design docs (`agentic-sdlc.md`, `agentic-sdlc-prd.md`, `agentic-sdlc-codedesigner.md`, `agentic-sdlc-marketresearcher.md`, `audit-event-shape.md`) all current. **Pending:** team-facing reading-order doc (where to start for a new team adopting this) + an in-extension `How to read an audit report` tooltip / quick-help panel.
- [x] **E6.** **Whole-OKR audit rollup** (added 2026-05-25 — not in original E plan; closed a UX bug where the OKR-detail footer's `📦 Export Audit Report` button was a disabled placeholder with `title="Phase E feature"`). `LookingGlassPanel.onExportOkrRollup(okrId)` + `services/AuditReportExporter.{buildOkrRollupMarkdown, computeOkrRollupVerdict, composeOkrRollupSourceTag, PhaseRollupDigest, OkrRollupInput}`. One auditor-grade markdown summarizing all 3 phases (WHY + HOW + WHAT) into `okrs/<id>/audit/exports/<okrId>-rollup.md`. **Verdict precedence**: FAIL > PARTIAL > PASS — PASS only when every completed phase is runner-verified + source-atomic + evidence-complete; PARTIAL when phases missing or runner not invoked for innocent reasons; FAIL on any missing-evidence / runner-FAIL / source-atomicity-broken. **No parallel verifier logic** — reuses `decideRunnerInvocation` + `verifyKeysAtomicity` + `fetchPrdAndArtifact` + `composeSourceTag` from the per-action path. **Hardening rounds closed**: E3-gold-r2 (canonical-runId derivation + runner FAIL semantics) · E3-gold-r3 (parseRunnerVerdictFromStdout extraction + source-atomicity guard + runner-driven exec summary) · E3-gold-r4 (per-input source tracking + GitHub keys atomicity + PRD/artifact suppress-non-canonical + bottom-up stdout walker) · E3-gold-r5 (structural `runnerInput` field closes the SHAPE-CLEARED-on-mismatch bug + chain-ladder suppression) · E3-gold-r5-followup (`composeSourceTag` predicate tightening — `runnerInput=not-applicable` only canonical when `keysSource=not-checked`) · E4-r1 (canonical-mode artifact check + per-phase Trust posture canonical-Sources + phase-label honesty) · E4-r1-followup (discriminated `getRepoFileStatus` so uncommitted local artifacts can't masquerade as canonical evidence).
...
**Closure (Bug V, 2026-05-23).** This reviewer-dispatch problem is retired, not pending. Separate `architect-reviewer` / `security-reviewer` agents were deleted from the template tree and `reviewer-bus.yml` is deprecated. PRD and code-design review now run as signed persona-switch self-critique inside the author agent. The old Tweedles goal — contrarian review pressure — is preserved by prompt packs and structured review events, not by separate Copilot sessions. The enforcement surface is now:

1. The author agent runs Architect and Security persona prompts in bounded rounds.
2. Each persona round writes a human-readable PR-body block and emits a signed `self_review` event while the per-epoch key is in scope.
3. The workflow parses those blocks only as a parity/readability surface; the signed chain event is authoritative.
4. Structure, citation, path-inventory, drift, manifest, chain, and seal gates remain deterministic workflow checks.

If future WHAT runs prove that self-critique misses code-grounded findings a separate reviewer would catch, that becomes a new design proposal. It is not part of the current shipped path.

#### 14.8.1 Retired reviewer-dispatch verification plan

The experiment below is no longer required. It was written when separate reviewer agents were still being considered. Bug V closed that path: reviewer dispatch is retired, reviewer agent templates are deleted, and author agents now run signed Architect + Security persona-switch reviews internally. Keep this heading only as a breadcrumb for readers following old PR links.
...
| `[x]` | **`code-design/synthesis.md`** prompt pack — emits ONE cross-cutting code-design doc grounded on PRD + indexed code repos | `vscode-extension/prompt-packs/looking-glass/code-design/synthesis.md` | D |
| `[x]` | **`code-design/architecture-review.md`** prompt pack — CODE-grounded architect review (CALM drift + interface contract diffs against actual code) | `vscode-extension/prompt-packs/looking-glass/code-design/architecture-review.md` | D |
| `[x]` | **`code-design/security-review.md`** prompt pack — CODE-grounded security review (OWASP pattern scan + threat-model compliance against actual code) | `vscode-extension/prompt-packs/looking-glass/code-design/security-review.md` | D |
