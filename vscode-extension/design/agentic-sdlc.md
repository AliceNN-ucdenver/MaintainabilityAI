...
- **Phase A** — **substantively shipped** with one open item: A12 Connect Repo flow on BAR detail (the deliverables-map row was always `[ ]`; the phase-plan list was missing the entry until now). The other 11 A-items + 5 beyond-plan extras shipped. The Connect Repo flow is ~half-day work; not blocking Phase B testing but blocks "Phase A 100% complete" claim.
- **Phase B** — agent path runnable end-to-end on Supervised + Restricted tiers. WHY merged cleanly (PR #103). HOW expected to merge cleanly with `prd-pass` on the next end-to-end test after the B25 fixes deploy. **Three real gaps before Phase B can be called complete:**
  1. **B5** — `context-architecture` / `context-security` / `context-quality` runtime backends are NOT shipped; the prd-agent's hard rule treats them as required. Latest HOW run only succeeded because the agent ignored its own contract. Marked `[!]` (blocked) below.
  2. **B14** — Court Recorder CloudEvents v1.0 envelope shipped as code but never adopted; on-disk JSONL is flat, not CloudEvents-wrapped. §11.2 + §15 both claim adoption. Marked `[~]` below.
  3. **B27** — Knight's Seal v1 (per-run ephemeral Ed25519 signing) — already planned and tracked.
- **Phase B drift items found by 2026-05-22 audit (UI lies that should be fixed in the next push):**
  - `okrDetail.ts:839` pre-flight signal advertises `architect-reviewer + security-reviewer` for HOW phase. Both retired by B24. (5-min fix.)
  - `lookingGlass.ts:1959-1960` Settings "What ships" list claims `architect-reviewer.yml` + `security-reviewer.yml` are deployed. They're in `DEPRECATED_MESH_FILES` and pruned on next Redeploy. (5-min fix.)
...
- HOW (PR #105) — passed audit but exposed the `A.false-audit-fabrication` threat (forged chain). Closed in B25 below. Re-run pending after the next deployment + B5 fix.

**True remaining work before "Phases A + B complete":**
1. B27 — Knight's Seal v1 (~1 day; full spec §11.5)
2. B5 — context-* runtime backends (~2h with stub-handler reusing knowledge-mesh-* reads)
3. B14 — wire CloudEvents envelope into runner OR strikethrough the §11.2 claim (~1h or ~5min)
4. DRIFT-1 + DRIFT-2 — fix two UI lies about reviewer agents (~10min combined)
5. A12 — Connect Repo flow (~half day; not blocking but completes Phase A)
6. B9 polish — Settings 3-tab refactor + deprecation notices (~3-4h; tracked `[~]` already)
...
- [ ] **A12.** **Connect Repo** button + flow for declared-but-not-connected `repos[]` entries on BAR detail — currently shows a placeholder text in `okrDetail.ts` ("Connection happens in Phase A-PR4 (Connect Repo flow on BAR detail)") but the flow itself is not implemented. Phase A is technically not 100% complete by its own deliverables-map row. ~half day to ship: BAR-detail UI for repo-status badges + Connect button that opens the repo's GitHub Settings → Actions allowlist page + saves the connected status on commit.
...
- [!] **B5.** Deploy context Skills: `context-architecture`, `context-security`, `context-quality` (pure mesh aggregators; agent applies persona) — **BLOCKED: runtime gap.** SKILL.md templates shipped in B-PR1 + deploy to mesh via MESH_SKILLS, but `packages/research-runner/src/runner/skills.ts` has NO handler for any of them. Runtime returns `{ok:false, reason:"unknown-skill: context-<x>"}`. The prd-agent's hard rule at `prd-agent.agent.md` line 158 states *"If a context-* Skill returns {ok: false}, stop. PRDs MUST be grounded."* — by the agent's own contract, every HOW run should halt at step 7. Latest clean HOW run (PR #105) only succeeded because the agent ignored its own hard rule and proceeded with reduced grounding. **Fix options:** (a) ship stub handlers that aggregate existing knowledge-mesh-* reads (~2h, reuses code), (b) relax the hard rule + emit `{ok: false, notes: "context-* unavailable in runner v0.1.x — proceeded with reduced grounding"}` audit events (~5min, honest-failure semantics). Recommendation: ship (a) so the contract is honored and the audit-honesty gate counts real mesh-skill calls.
...
- [~] **B14.** Court Recorder CloudEvents v1.0 emitter — **shipped but UNADOPTED.** `packages/research-runner/src/runner/court-recorder.ts` exports `buildCloudEventsEnvelope` + `serializeCloudEventsEnvelope` with passing tests. BUT `handleAuditEmitEvent` in `skills.ts` writes a flat JSONL schema (event_id / event_kind / payload / prev_event_hash / event_hash) — NEVER calls the CloudEvents composer. §11.2 + §15:2656 both claim CloudEvents 1.0 adoption; reality is dead code. **Fix options:** (a) wire the composer into the runner so on-disk JSONL becomes CloudEvents 1.0 (~1h — compose envelope around the existing draft before hashing), (b) strikethrough the §11.2 + §15 CloudEvents claims and rename B14 to "envelope helper for future SIEM exports — not on-disk format" (~5min). Recommendation: ship (a) before claiming Phase B complete since the design doc + threat model both reference CloudEvents.
...
| `[~]` | **Court Recorder** CloudEvents v1.0 envelope emitter — **shipped but UNADOPTED.** `buildCloudEventsEnvelope` + `serializeCloudEventsEnvelope` exported from `court-recorder.ts` with passing tests; `handleAuditEmitEvent` in `skills.ts` writes flat JSONL and never calls them. Fix: wire the composer into the runner OR strikethrough the §11.2 + §15 CloudEvents claim. See §13 B14 entry. | `packages/research-runner/src/runner/court-recorder.ts` (helper) + `packages/research-runner/src/runner/skills.ts` (needs composition) | B |
