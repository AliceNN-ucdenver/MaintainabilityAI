...
- [~] **B5.** Deploy context Skills: `context-architecture`, `context-security`, `context-quality` (pure mesh aggregators; agent applies persona) — SKILL.md templates shipped in B-PR1; CLI backends deferred to Phase D alongside the code-grounded reviewer prompt packs (these Skills are reviewer-only, and the reviewers are not yet dispatch-stable per the PRD-reviewer concern in §14.x).
...
- [x] **B20.** Per-agent workflow consolidation + user-triggered label flow (B-PR1k). Pivots the agentic-SDLC workflow architecture from concern-keyed (`pr-auto-label.yml`, `audit-validate.yml`, `drift-gate.yml`, `reviewer-bus.yml`, `okr-state-machine.yml`, `design-bus.yml`, `okr-bus.yml`) to **agent-keyed** — one workflow file per `.github/agents/<name>.agent.md` declaration, with multi-trigger jobs covering the agent's complete lifecycle. Two coupled changes:
  ...
  **(a) Workflow consolidation: 13 files → 6 files.**
  ...
  | New | Replaces | Lifecycle handled |
  |---|---|---|
  | `market-research-agent.yml` | okr-bus (WHY branch) + audit-validate (WHY branch) + drift-gate (WHY branch) | issue dispatch on `oraculum-research` + audit-and-drift on `research-synthesis` label + finalize on merge |
  | `prd-agent.yml` | okr-bus (HOW) + audit-validate (HOW) + drift-gate (HOW) + okr-state-machine (HOW) | dispatch + audit + state-machine (governance-pass promotion, revision-escalation, HumanGate) + finalize |
  | `code-design-agent.yml` | okr-bus (WHAT) + audit-validate (WHAT) + drift-gate (WHAT) + okr-state-machine (WHAT) + design-bus | dispatch + audit + state-machine + per-repo fanout on merge |
  | `architect-reviewer.yml` | reviewer-bus (architect branch) | dispatch on `prd-draft`/`design-draft` + parse structured-review comment into `governance-pass-architecture` / `revision-required` |
  | `security-reviewer.yml` | reviewer-bus (security branch) | same pattern, security pack |
  | `oraculum-review.yml` | UNCHANGED — BAR-page architecture review (separate domain) | — |
