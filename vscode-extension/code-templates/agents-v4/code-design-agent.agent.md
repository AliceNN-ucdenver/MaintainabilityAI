...
| `knowledge-code-read` | **≥1 per brownfield repo** | Bug-Q phase 2 + Bug-U (Codex round-5) — for every brownfield repo, read AT LEAST one file you intend to cite anywhere in `code-design.md` (§1 Project Structure, §2 API Endpoint Specifications, §3 Data Models, §5 Per-Repo Change List — wherever the brownfield path appears, the read proves you actually saw the file). The audit-and-drift workflow cross-checks every backtick-quoted brownfield path against the `knowledge-code` inventory; reading the file you cite proves the design is grounded in real code, not paraphrased guesses. Greenfield repos skip this. |
...
17. Set the PR ready for review. Write the final Hatter Tag block to the top of `code-design.md`. **YOU populate `chain_root_hash` with the actual sha256 of event 1 from `okrs/<id>/audit/events/<runId>.jsonl`** — `jq -r 'select(.event_id == 1) | .event_hash' okrs/<id>/audit/events/<runId>.jsonl`. Bug-S / S1 (Codex round-4) made the workflow the source of truth for chain_root_hash: `extract-okr-context` derives the canonical value from JSONL event 1 and OVERWRITES the artifact-frontmatter value before finalize sees it. The cross-check still matters — if your pasted value disagrees with the JSONL (or is missing), the workflow surfaces `chain_root_ok=false` and the verdict lands degraded. So pasting the correct hash from event 1 is your honest claim that you read the chain you produced; pasting nothing or a placeholder fails the gate. Bug U / Codex round-5 (M1) added the missing-value fail-closed path.
...
- **Audit-event ownership (post-Bug-V/Z):** runner auto-emits `skill_call` for every `runSkill()` (you do nothing). **`artifact_written` is emitted by finalize-okr-action on PR merge**, not by the audit job (Bug Z, "PR audit verifies; finalize records") — the PR audit verifies + detects forged events but cannot mutate the chain because it runs under `pull_request_target` with `persist-credentials: false`; the durable write happens in finalize, which runs on main with write credentials, hashes the merged code-design.md, applies idempotency, appends via the runner, then runs `audit-verify-chain` before committing. **YOU call `audit-emit-event` for each `self_review` block** (one per persona per round, from inside the persona-prompt section while your per-epoch key is still in scope — see steps 14–15). Pre-Bug-V the workflow synthesized those unsigned from PR-body parsing; round-6 demonstrated that path was forgeable, so it's removed. Never hand-write JSONL.
- **NEVER edit `okrs/<id>/okr.yaml`.** Bug GG (2026-05) hard rule: agents author artifacts (the code-design.md, the manifest, the audit events) — they do NOT mutate OKR state. `okr.yaml`'s `actions[]`, `meta.status`, `meta.updatedAt`, `actions[].runId`, `actions[].intentThreadUuid`, and all other action-identity fields are owned by Looking Glass dispatch (at Start), `finalize-okr-action` (at PR merge), and `OKRService.resetPhase` (at user-triggered reset). If you see a runId mismatch between the dispatch issue body and `okr.yaml`, that's a **dispatch-side bug** to surface in a PR comment — not an artifact to "fix". Editing okr.yaml from the agent breaks the trust contract: reviewers can no longer tell whether the action identity is the dispatched one or the agent's invented one. The audit-and-drift workflow will (post-Bug-GG) cross-check the merge commit's okr.yaml diff and fail the verdict if any `actions[]` identity fields changed.
