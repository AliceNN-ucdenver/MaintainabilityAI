...
- **NEVER edit `okrs/<id>/okr.yaml`.** Bug GG (2026-05) hard rule: agents author artifacts (the PRD, the manifest, the audit events) — they do NOT mutate OKR state. `okr.yaml`'s `actions[]`, `meta.status`, `meta.updatedAt`, `actions[].runId`, `actions[].intentThreadUuid`, and all other action-identity fields are owned by Looking Glass dispatch (at Start), `finalize-okr-action` (at PR merge), and `OKRService.resetPhase` (at user-triggered reset). If you see a runId mismatch between the dispatch issue body and `okr.yaml`, that's a **dispatch-side bug** to surface in a PR comment — not an artifact to "fix". Editing okr.yaml from the agent breaks the trust contract: reviewers can no longer tell whether the action identity is the dispatched one or the agent's invented one. PR #140 (2026-05-25) tripped this: the agent saw `okr.yaml.actions[].runId = HOW-2026-05-24-b6nq18` (stale, from a deleted prior run) and the dispatch issue showing `HOW-2026-05-25-j2gpke` (new run), so it edited okr.yaml to "fix" the mismatch. That edit was a governance violation — the right move was to leave okr.yaml alone, complete the PRD (which IS your job), and post a PR comment naming the upstream stale-action bug so a human or Looking Glass-Reset could clean it up. The audit-and-drift workflow will (post-Bug-GG) cross-check the merge commit's okr.yaml diff and fail the verdict if any `actions[]` identity fields changed.
...
For WHAT phase (Phase 3, code-design-agent), reviewers READ ACTUAL CODE — a different evidence surface from the author. There the independence axis is real in principle, BUT the D8 reviewer-dispatch open decision was **closed 2026-05-23 (Bug V)** in favor of persona-switch self-critique for every phase including WHAT — "less brittle keeping it contained with the agent." The `code-design-agent` inhabits Code-Architect + Code-Security personas via the same B24 pattern. The separate-reviewer `.agent.md` files were deleted; see [`agentic-sdlc-codedesigner.md` §D8 — CLOSED](../../design/agentic-sdlc-codedesigner.md) for the closure record.
