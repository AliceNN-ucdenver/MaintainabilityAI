...
- **Non-functional requirements** must be marked with `**NFR-NN**` (bold) OR `### NFR-NN` / `### NFR-NN:` (H3 heading). Freeform bullets without an `NFR-NN` marker are not governed requirements and fail the audit.
...
- **NFR citations:** within 400 characters of each NFR marker, there MUST be at least one `S-N`, `C-N`, `R-N`, or `E-N` tag. The Quality persona owns this section; every performance, reliability, operability, or maintainability requirement needs a durable upstream reason, not just a nice-sounding target.
...
- Every NFR MUST be numbered `NFR-NN` and cite at least one `S-N`, `C-N`, `R-N`, or `E-N` tag within 400 chars of the NFR marker. Do not write unnumbered Quality bullets; they will show up as `0 NFR` in the dashboard and fail the structure gate.
