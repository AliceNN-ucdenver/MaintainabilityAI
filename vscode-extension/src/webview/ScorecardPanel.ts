...
export class ScorecardPanel extends BasePanel<ScorecardWebviewMessage, ScorecardExtensionMessage> {
  ...
  private async onCheckAliceWorkflowStatus() {
    ...
    try {
      const exists = await this.githubService.checkWorkflowExists(
        ...
        this.currentRepo.repo,
        '.github/agents/alice-maintenance-agent.agent.md',
      );
  ...
  private async onDeployAliceWorkflow() {
    ...
    this.postMessage({ type: 'loading', active: true, message: 'Deploying Alice maintenance agent...' });
    ...
    try {
      ...
      // 4. Write the Alice maintenance-agent persona
      const agentsDir = path.join(gitRoot, '.github', 'agents');
      fs.mkdirSync(agentsDir, { recursive: true });
      const content = generateMaintenanceAgent(this.context.extensionPath);
      fs.writeFileSync(path.join(agentsDir, 'alice-maintenance-agent.agent.md'), content, 'utf8');

      // 5. Surgical git add, commit, push
      await execFileAsync('git', ['add', '.github/agents/alice-maintenance-agent.agent.md'], { cwd: gitRoot });
      const { stdout: diffCheck } = await execFileAsync('git', ['diff', '--cached', '--name-only'], { cwd: gitRoot });
      if (diffCheck.trim()) {
        await execFileAsync('git', ['commit', '-m', 'chore: deploy Alice maintenance agent\n\nGenerated by MaintainabilityAI VS Code Extension'], { cwd: gitRoot });
