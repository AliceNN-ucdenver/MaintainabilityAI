...
export class LookingGlassPanel extends BasePanel<LookingGlassWebviewMessage, LookingGlassExtensionMessage> {
  ...
  private readonly messageHandlers: MessageHandlers<LookingGlassWebviewMessage> = {
    ...
    // The issue-based research dispatch (research-request label →
    // archeologist.yml) was retired with the pruned workflows. Research
    // returns as a governed agent with skills — design/research-agent-
    // alignment.md captures the intent + migration. Buttons stay as
    // discoverable placeholders.
    'newResearchFromPlatform':     () => {
...
`;
  ...
  private async onCreateResearchSecret(id: ResearchSecretId) {
    ...
    const instructions =
      ...
      `  • Issues   = Read and write   (mesh workflows open + update landing-issues in code repos)\n` +
      `  • Contents = Read       (lets workflows cross-reference code-repo files in the issue body)\n\n` +
      `Account permissions:\n` +
      `  • Models   = Read       (agent workflows route LLM calls through GitHub Models — using your Copilot tier instead of the free-tier 8K cap)\n\n` +
