...
`;
  ...
  private async onProvisionWorkflow() {
    ...
    try {
      ...
      if (changedFiles.length > 0) {
        ...
        toastMessage = `Redeployed ${written.length} workflow file(s) and ${changedFiles.length - written.length >= 0 ? changedFiles.length - written.length : 0} prompt file(s) to the mesh.`;
      } else {
        toastMessage = `Workflows already up-to-date — nothing to commit.`;
      }
  ...
  private async onRefreshPromptPacks() {
    ...
    try {
      ...
      // Commit + push so GH Actions workflows that clone the mesh actually
      // see the new prompt versions. Prior bug: local refresh happened but
      // nothing was pushed, so research runs kept using the old prompts.
      ...
      if (changed.length > 0) {
        ...
        toast = `Refreshed ${changed.length} prompt file(s) and pushed to the mesh repo.`;
      } else {
