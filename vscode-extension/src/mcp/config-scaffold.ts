...
/**
 * Bug-AAE Phase 2/3 — STANDALONE implementation provenance gate.
 *
 * Scaffolded UNCONDITIONALLY (NOT gated on `agent_type`, unlike the
 * review workflow): chain provenance is orthogonal to which AI reviews
 * PRs, so a repo whose mesh hasn't configured a review framework still
 * gets its impl PRs verified. Runs on impl PRs (head branch
 * `copilot/okr-*`) and verifies the signed audit chain + required
 * skill_call manifest + Hatter Tag, and surfaces the Red Queen decision
 * log. REUSES the runner's skill-audit-verify-chain (one verifier, the
 * same code path the runner signs with — no hand-rolled Ed25519). Other
 * PRs skip the job via the head-ref guard.
 */
...
export function scaffoldAgentConfig(
  ...
  // Bug-AAE Phase 2/3 — the implementation provenance gate is UNCONDITIONAL
  // (not gated on agent_type like the review workflow): chain provenance
  // must verify on every impl PR regardless of whether a review framework
  // is configured.
