...
export function generateImplProvenanceWorkflow(): string {
  ...
  lines.push(`          # Tier 2.5a — scan the IMPL events JSONL for the rolled-up`);
  lines.push(`          # redqueen_decisions digest event the runner emits as the agent's`);
  lines.push(`          # FINAL governed action. HONESTY (Codex finding 1): the mere`);
  lines.push(`          # PRESENCE of the event proves nothing cryptographically — a`);
  lines.push(`          # hand-written line would also set it. The only thing this step`);
  lines.push(`          # can prove on its own is rq_digest_match: the event's signed`);
  lines.push(`          # log_sha256 equals the digest of the committed log. The event's`);
  lines.push(`          # SIGNATURE itself is verified by the 'chain' step above`);
  lines.push(`          # (skill-audit-verify-chain), so rq_verified = chain_ok AND`);
  lines.push(`          # digest_match. Both ADVISORY (back-compat: absent on older`);
  ...
  lines.push(`          print('red queen decisions: allowed=%d denied=%d present=%s digest_present=%s digest_match=%s' % (allowed, denied, present, digest_present, digest_match))`);
  lines.push(`          PYEOF`);
  lines.push(``);
  ...
  lines.push(`            // Tier 2.5a — advisory status, HONEST about strength (Codex finding 1):`);
  lines.push(`            // a digest event's signature is verified by the 'chain' step above, so`);
