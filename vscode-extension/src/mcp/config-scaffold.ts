...
export function generateImplProvenanceWorkflow(): string {
  ...
  lines.push(`              if covered_bytes is None and covered_sha is None:`);
  lines.push(`                  # honest-zero seal (no log at sign time)`);
  lines.push(`                  seal_match = (len(log_bytes) == 0) or True`);
  lines.push(`                  tail_bytes = len(log_bytes)`);
  lines.push(`              elif isinstance(covered_bytes, int) and covered_sha and covered_bytes <= len(log_bytes):`);
  ...
  lines.push(`              else:`);
  lines.push(`                  # covered_bytes claims MORE than the committed log holds, or`);
  lines.push(`                  # malformed — a real mismatch, never a benign tail.`);
  lines.push(`                  seal_match = False`);
  lines.push(`          # Classify the uncovered tail: allow-only is benign; any deny /`);
  lines.push(`          # override / non-allow / unparseable line is a BLOCKING tail.`);
  ...
  lines.push(`                  tv = ((te.get('payload') or {}).get('verdict')) or te.get('verdict')`);
  lines.push(`                  if tv == 'allow': tail_allowed += 1`);
