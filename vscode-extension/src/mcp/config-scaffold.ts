...
export function generateImplProvenanceWorkflow(): string {
  ...
  lines.push(`              # (the runner emits covered_bytes=0, covered_sha256=null). A`);
  lines.push(`              # zero-byte prefix is vacuously sealed, so the WHOLE committed log`);
  lines.push(`              # is the uncovered tail (the agent's own post-seal commit`);
  lines.push(`              # decisions). NOTE: must test covered_bytes == 0, NOT 'is None' —`);
  lines.push(`              # the runner sends the int 0, not null, for this case.`);
  lines.push(`              if isinstance(covered_bytes, int) and covered_bytes == 0:`);
