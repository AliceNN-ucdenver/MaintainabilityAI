...
/**
 * Alice — the Cheshire maintenance agent persona. Replaces the retired
 * `alice-remediation.yml` (a `claude-code-action` workflow + ANTHROPIC_API_KEY).
 * Alice is a custom Copilot persona the CodeQL→issues automation (and the
 * Looking Glass / Scorecard dispatch) assigns by name; she reads the
 * `.cheshire/prompts/` packs + `.github/repo-metadata.yml` and grounds her work
 * in the real code, governed by the baked Red Queen policy + PreToolUse hook.
 */
export function generateMaintenanceAgent(extensionPath: string): string {
  return readScaffoldFile(extensionPath, 'agents', 'alice-maintenance-agent.agent.md');
}
...
/**
 * Code-repo files Cheshire used to scaffold but no longer does —
 * `pruneDeprecatedCodeFiles` deletes any that still exist on the next
 * Scaffold/Deploy so already-scaffolded repos converge on the v2 layout.
 * Mirrors {@link DEPRECATED_MESH_FILES} for the code-repo side.
 *
 * Added Cheshire v2 (2026-06-11): the `claude-code-action` remediation path
 * (`alice-remediation.yml` + `CLAUDE.md`) is replaced by the Alice persona,
 * and the MCP "live mesh tools" layer (`.mcp.json` + `.redqueen/mcp-runner.js`
 * + `.github/copilot-governance-steps.yml`) is retired in favor of the baked
 * deterministic policy. Never remove an entry once added (idempotent prune).
 */
export const DEPRECATED_CODE_FILES: string[] = [
  // ── Cheshire v2 (2026-06-11) — Alice persona replaces the claude path ──
  'CLAUDE.md',
  '.github/workflows/alice-remediation.yml',
  // ── MCP "live mesh tools" layer retired (deterministic policy instead) ──
  '.mcp.json',
...
/**
 * Delete superseded Cheshire-scaffolded files from a code repo's working
 * tree. Returns the paths actually removed (missing paths are skipped). The
 * caller commits + pushes — this only touches the working tree.
 */
export function pruneDeprecatedCodeFiles(repoRoot: string): string[] {
  const removed: string[] = [];
  for (const relativePath of DEPRECATED_CODE_FILES) {
