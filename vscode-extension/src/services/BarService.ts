...
export class BarService {
  ...
  scorePillars(barPath: string, platformId: string, platformName: string): BarSummary | null {
    ...
    const pillarTrends = {
      architecture: BarService.computeGovernanceTrend(scoreHistory, s => s.architecture),
      security: BarService.computeGovernanceTrend(scoreHistory, s => s.security),
      infoRisk: BarService.computeGovernanceTrend(scoreHistory, s => s.information_risk),
      operations: BarService.computeGovernanceTrend(scoreHistory, s => s.operations),
    };
  ...
  /**
   * Read governance score history from score-history.yaml.
   */
  readScoreHistory(barPath: string): GovernanceScoreSnapshot[] {
    const yamlPath = path.join(barPath, 'score-history.yaml');
    if (!fs.existsSync(yamlPath)) { return []; }

    try {
      const content = fs.readFileSync(yamlPath, 'utf8');
      return this.parseScoreHistoryYaml(content);
    } catch {
      return [];
    }
  }
  ...
  /**
   * Append a governance score snapshot to score-history.yaml.
   * Only appends if the score differs from the last snapshot.
   */
  appendScoreSnapshot(barPath: string, snapshot: GovernanceScoreSnapshot): void {
    ...
    const yamlPath = path.join(barPath, 'score-history.yaml');
    const entry = `  - timestamp: "${snapshot.timestamp}"\n    composite: ${snapshot.composite}\n    architecture: ${snapshot.architecture}\n    security: ${snapshot.security}\n    information_risk: ${snapshot.information_risk}\n    operations: ${snapshot.operations}`;

    let content: string;
    if (fs.existsSync(yamlPath)) {
      content = fs.readFileSync(yamlPath, 'utf8').trimEnd() + '\n' + entry + '\n';
    } else {
      content = `# Governance Score History — generated by MaintainabilityAI\nsnapshots:\n${entry}\n`;
    }
   ...
   */
  static computeGovernanceTrend(
    history: GovernanceScoreSnapshot[],
    extract: (s: GovernanceScoreSnapshot) => number
  ): GovernanceTrend {
  ...
  private parseScoreHistoryYaml(content: string): GovernanceScoreSnapshot[] {
    const snapshotsIdx = content.indexOf('snapshots:');
    if (snapshotsIdx === -1) { return []; }

    const after = content.slice(snapshotsIdx);
    const snapshots: GovernanceScoreSnapshot[] = [];
    ...
    for (let i = 1; i < blocks.length; i++) {
      ...
      snapshots.push({
        timestamp,
        composite: getNum('composite'),
        architecture: getNum('architecture'),
        security: getNum('security'),
        information_risk: getNum('information_risk'),
